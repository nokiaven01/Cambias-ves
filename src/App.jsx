...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .rate-icon { width:30px; height:30px; border-radius:9px; display:flex; align-items:center; justify-content:center; flex-shrink:0; overflow:hidden; }
  ...
  .rate-img { width: 22px; height: 22px; object-fit: contain; }
...
async function fetchRates() {
  ...
  // 0. FUENTE OFICIAL PROPIA: /api/bcv (función serverless de Vercel)
  try {
  ...
  // 2. FALLBACK USD+EUR: jsDelivr CDN
  if (!fromApi) {
    try {
  ...
  // 3. FALLBACK USD: ve.dolarapi.com
  ...
  // 4. FALLBACK EURO: bcv-api.deno.dev
  ...
  // 5. USDT via Criptoya (Binance P2P)
  try {
...
function formatVEInput(amount) {
  ...
  if (decStr === undefined) return intFmt;
  return `${intFmt},${decStr.slice(0, 2)}`;
}
...
export default function App() {
  ...
  /* ─── DEFINICIÓN DE TARJETAS DE COTIZACIÓN CON IMÁGENES OFICIALES ─── */
  const rateCards = [
    { 
      id: "bcv",          
      image: "https://upload.wikimedia.org/wikipedia/commons/e/e0/Banco_Central_de_Venezuela_logo.png", 
    ...
    { 
      id: "euro",         
      image: "https://upload.wikimedia.org/wikipedia/commons/b/b7/Euro_symbol.svg", 
      alt: "Logo Euro",
      name: "Euro BCV",             
      subtitle: "Cotización oficial EUR",     
      value: rates.euro,         
      unit: "Bs/EUR",  
      src: "bcv.today"    
    },
            ...
            <div key={card.id} className={`rate-card ${card.id}`}>
              <div className="rate-left">
                <div className={`rate-icon ${card.id}`}>
                  <img src={card.image} alt={card.alt} className="rate-img" />
                </div>
                <div>
              ...
              {rateCards.map(card => (
                ...
                >
                  <div className="pm-rate-name">
                    <img src={card.image} alt={card.alt} style={{ width: 14, height: 14, objectFit: "contain" }} /> {card.name}
                  </div>
